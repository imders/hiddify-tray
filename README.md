...
**При входе в систему Hiddify разворачивает окно.** Скрипт прячет его сам — за это отвечает
пункт меню «Прятать окно при запуске». Прячется только окно, появившееся в первые 45 секунд
после старта приложения (`HideWindowMs`): всё, что вы открыли позже или через меню трея,
не трогается. Вернуть окно — «Открыть окно Hiddify».

У самого Hiddify есть настройка «Тихий запуск» (Настройки → Общие), но включить её можно
только в интерфейсе: соответствующего ключа в `shared_preferences.json` не появляется,
и добавление `silent-start` вручную ничего не даёт — приложение его не читает.
