...
- **Прятать окно при запуске** — убирать окно, которое Hiddify разворачивает сам при старте *(включено)*
...
**При входе в систему Hiddify разворачивает окно.** Лечится настройкой самого приложения:
**Настройки → Общие → «Запускать свернутым»**. Другого надёжного способа нет.

Снаружи это окно закрыть не получится, и пробовать не стоит — проверено замерами:
`WinHide` выполняется без ошибки, но окно остаётся видимым, а `WinMinimize` разворачивается
обратно примерно через секунду. Окно нарисовано на Flutter, и приложение восстанавливает его
само (по той же причине не проходит и синтетический клик по кнопке подключения).

Пункт меню «Прятать окно при запуске» делает несколько попыток на самом старте — иногда они
успевают сработать, пока приложение не закончило инициализацию. Попыток намеренно всего
четыре (`HideTries`): без ограничения скрипт боролся бы с приложением, и окно бы мигало.
Рассчитывать на это не нужно, настройка «Запускать свернутым» надёжнее.
